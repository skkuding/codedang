...
data "aws_iam_policy_document" "client_api_ses_send" {
  statement {
    actions = [
      "ses:SendEmail",
      "ses:SendRawEmail",
...
resource "aws_iam_user_policy" "client_api_ses_send" {
  name   = "codedang-client-api-ses-send"
  user   = aws_iam_user.client_api.name
  policy = data.aws_iam_policy_document.client_api_ses_send.json
}
