...
n8n currently runs as a private, locally managed Helm release in the production cluster. `../argocd/applications/n8n.yaml` defines future ArgoCD tracking with self-heal enabled and pruning disabled, but intentionally points to the unpushed local branch. Move it to a private Git remote before applying it. The existing SQLite database must be copied to the PVC before upgrading the release.
