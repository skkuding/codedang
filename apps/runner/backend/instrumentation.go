...
const backendServiceName = "RUNNER_BACKEND"
...
func initInstrumentation(
	...
	instrumentationOnce.Do(func() {
		...
		metricProvider := otelmetric.NewMeterProvider(
			...
			otelmetric.WithReader(otelmetric.NewPeriodicReader(metricExporter, otelmetric.WithInterval(3*time.Second))),
		)
		otel.SetMeterProvider(metricProvider)
		shutdownFuncs = append(shutdownFuncs, metricProvider.Shutdown)

		logExporter, err := otlploggrpc.New(ctx, otlploggrpc.WithEndpoint(otlpEndpoint), otlploggrpc.WithInsecure())
		if err != nil {
