...
func (p *Postgres) GetAllCodes(problemId string, language string) ([]Element, error) {
  rows, err := p.client.Query(`SELECT id, userId, code, createTime FROM public.submission WHERE problem_id = $1 language = $2`, problemId, language)
}
